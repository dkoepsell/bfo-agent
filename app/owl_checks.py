...
def _is_owl_vocab(iri: str) -> bool:
    """Exactly the owl namespace plus one clean token (owl#Thing,
    owl#disjointWith, ...). 'owl#complementOf working:X' does NOT qualify."""
    return (iri.startswith(_OWL_NS)
...
def check_expression_iris(raw_text: str) -> Report:
    ...
    for m in _IRI_ATTR.finditer(raw_text):
        iri = m.group(1)
        if _is_owl_vocab(iri):
            continue
        frag = _fragment_of(iri)
        # A ':' in the fragment means a QName was smuggled into a full IRI --
        # except a '_:' blank-node label, which check_bnode_iris owns.
        qname_in_frag = (":" in frag and not frag.startswith("_:")
                         and not _HOST_PORT.match(frag))
        if ("[" in iri or "]" in iri
                or any(t in iri for t in _EXPR_TOKENS)
                or any(t in frag for t in _OWL_CONSTRUCTS)
                or qname_in_frag):
